...
namespace VocabLab.Services
{
    public class WordService : IWordService
    {
        ...
        public WordService(AppDbContext context) => _context = context;
        ...
        public async Task<IEnumerable<Word>> GetByCategoryAsync(string category) =>
            await _context.Words.Where(w => w.Category == category).ToListAsync();

        public async Task<List<QuizQuestion>> GenerateQuizAsync(string userId, int count = 10)
        {
        ...
        private List<string> GenerateOptions(Word correct, List<Word> allWords)
        {
            var wrong = allWords
            ...
            return wrong.OrderBy(x => Guid.NewGuid()).ToList();
        }

        public async Task SaveProgressAsync(string userId, int wordId, bool isCorrect)
        {
            ...
            if (progress == null)
            {
                progress = new UserWordProgress { UserId = userId, WordId = wordId };
            ...
            progress.IsLearned = progress.CorrectCount >= 3;
